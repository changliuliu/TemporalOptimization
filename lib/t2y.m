function [refy,A,b,varargout] = t2y(t,path,theta,vref,varargin)
if ~isempty(varargin)
    v0 = varargin{1};
    t0 = varargin{2};
else
...
for i=1:nstep
    ...
    if i > 2
        ...
        dh(3,i) = t(i-1)*t(i-2)*(t(i-1)+t(i-2))*(t(i)^2+t(i-1)*t(i)+(2*t(i)+t(i-1))*(t(i)+t(i-1)+t(i-2)));
        dh(3,i-1) = t(i)*t(i-2)*(t(i-1)^3+t(i-1)^2*(t(i)+t(i-2))+t(i)*t(i-1)*t(i-2)+(3*t(i-1)^2+2*t(i-1)*(t(i)+t(i-2))+t(i)*t(i-2))*(t(i)+t(i-1)+t(i-2)));
        dh(3,i-2) = t(i)*t(i-1)*(t(i)+t(i-1))*(t(i-2)^2+t(i-1)*t(i-2)+(2*t(i-2)+t(i-1))*(t(i)+t(i-1)+t(i-2)));
        ...
        if i == 1
            ...
            df(3,1) = 6*v0(1);
            dh(3,1) = 3*t(1)^2+6*t(1)*t0+2*t0^2;
            
            df(4,1) = 6*v0(2);
            dh(4,1) = 3*t(1)^2+6*t(1)*t0+2*t0^2;
        else
            ...
            dh(3,i) = t(i-1)*t0*(t(i-1)+t0)*(t(i)^2+t(i-1)*t(i)+(2*t(i)+t(i-1))*(t(i)+t(i-1)+t0));
            dh(3,i-1) = t(i)*t0*(t(i-1)^3+t(i-1)^2*(t(i)+t0)+t(i)*t(i-1)*t0+(3*t(i-1)^2+2*t(i-1)*(t(i)+t0)+t(i)*t0)*(t(i)+t(i-1)+t0));
